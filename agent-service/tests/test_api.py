...
def _make_auth_header() -> dict[str, str]:
    ...
    secret = settings.jwt_secret or (
        "9a4f2c5d6e7a8b9c0d1e2f3a4b5c6d7e8f9a0b1c2d3e4f5a6b7c8d9e0f1a2b3c"
    )
    key = base64.b64decode(secret)
    token = jwt.encode(
        {"sub": "test@digitalia.ma", "role": "RECRUITER"},
        key,
        algorithm="HS256",
    )
...
def test_search_endpoint():
    ...
    with patch("src.api.routes.run_sourcing_agent", mock_agent):
        ...
        # Verify parameters passed through to run_sourcing_agent.
...
@pytest.mark.asyncio
@patch(
    "src.agent.graph.store_candidate_pool_batch",
    new_callable=AsyncMock,
)
...
@patch(
    "src.agent.graph.store_candidate_pool_batch",
    new_callable=AsyncMock,
    side_effect=Exception("DB pool down"),
)
