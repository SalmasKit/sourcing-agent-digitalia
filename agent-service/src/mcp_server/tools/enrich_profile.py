...
async def enrich_candidate(linkedin_url: str, snippet_hint: str = "") -> EnrichedProfile | None:
    ...
    try:
        ...
        logger.info(f"[enrich_candidate] Attempting Apollo enrichment for {url}")

        # --- Check cache first — free, and avoids wasting quota on repeat lookups ---
        ...
        # --- Quota check (atomic DB upsert) — only for genuine new lookups ---
        allowed = await QuotaManager.check_and_increment()
        if not allowed:
            status = "quota_exceeded"
            apollo_quota_rejections.inc()
            logger.warning("[enrich_candidate] Monthly enrichment quota exceeded.")
            return None
